...
import sys
from validation.validation_lq import data_utils
...
def infer(model, dataloader, gpu_id, use_flip_test, fusion_method):
    ...
    with torch.no_grad():
        for images, labels, dataname, image_index in tqdm(dataloader):
            ...
            outcome_feature, outcome_norm = infer_batch(model, use_flip_test, fusion_method, images)
...
def infer_batch(model, use_flip_test, fusion_method, images):
    ...
    if use_flip_test:
        ...
        stacked_embeddings = torch.stack([outcome_feature, flipped_feature], dim=0)
        if outcome_norm is not None:
            stacked_norms = torch.stack([outcome_norm, flipped_norm], dim=0)
        else:
            stacked_norms = None

        outcome_feature, outcome_norm = fuse_features_with_norm(stacked_embeddings, stacked_norms, fusion_method=fusion_method)
...
if __name__ == "__main__":
    ...
    tinyface_test = tinyface_helper.TinyFaceTest(tinyface_root=args.data_root, alignment_dir_name="aligned_pad_0.1_pad_high")
