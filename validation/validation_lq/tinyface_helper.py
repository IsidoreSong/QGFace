...
class TinyFaceTest:
    def __init__(self, tinyface_root="/data/data/faces/tinyface", alignment_dir_name="aligned_pad_0.1_pad_high"):
        ...
        self.proto_gal_paths = [
            os.path.join(tinyface_root, alignment_dir_name, "Gallery_Match", p[0].item()) for p in self.gallery_dict["gallery_set"]
        ]
    ...
    def test_identification(self, features, ranks=[1, 5, 20], gpu_id=None):
...
def DIR_FAR(score_mat, label_mat, ranks=[1], FARs=[1.0], get_false_indices=False, gpu_id=None):
    ...
    #         sort_idx_mat_m_G_one = torch.argsort(score_mat_m_G[i:i + chunk_size, :], dim=1)
    #         sort_idx_mat_m_list.append(sort_idx_mat_m_G_one.cpu().numpy())
    #     sort_idx_mat_m = np.concatenate(sort_idx_mat_m_list)
    # else:
    # chunk_size = 10000
    # sort_idx_mat_m_list = []
    # for i in range(0, score_mat_m.shape[0], chunk_size):
    #     sort_idx_mat_m_G_one = np.argsort(score_mat_m[i:i + chunk_size, :], axis=1)
    #     sort_idx_mat_m_list.append(sort_idx_mat_m_G_one)
    # sort_idx_mat_m = np.concatenate(sort_idx_mat_m_list)
    ...
    for row in range(label_mat_m.shape[0]):
        ...
        if sort_idx.max() > label_mat_m.shape[1]:
            print("error")
            exit(1)
