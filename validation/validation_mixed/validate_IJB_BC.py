...
from validation.validation_mixed.insightface_ijb_helper.dataloader import (
    prepare_dataloader,
)
from validation.validation_mixed.insightface_ijb_helper import (
    eval_helper_identification,
)
from validation.validation_mixed.insightface_ijb_helper import (
...
def fuse_features_with_norm(
    ...
    if fusion_method == "norm_weighted_avg":
        ...
        fused = torch.cat(
            [stacked_embeddings[0], stacked_embeddings[1]], dim=-1
        )
        if stacked_norms is None:
            fused_norm = torch.ones((len(fused), 1))
        else:
    ...
    with torch.no_grad():
        for images, idx in tqdm(dataloader):
            ...
            if use_flip_test:
                ...
                flipped_feature = model(
                    fliped_images.to("cuda:{}".format(gpu_id))
                )
...
def infer(model, dataloader, gpu_id, use_flip_test, fusion_method):
    model.eval()
    features = []
    norms = []
    with torch.no_grad():
        for images, labels, dataname, image_index in tqdm(dataloader):
            images = images.to(f"cuda:{gpu_id}")
            outcome_feature, outcome_norm = infer_batch(
                model, use_flip_test, fusion_method, images
            )
            # print(images.sum().item(), outcome_feature.sum().item(), outcome_feature.min().item(), outcome_feature.max().item())
            features.append(outcome_feature)
            norms.append(outcome_norm)
    features = torch.cat(features, dim=0).cpu().numpy()
    norms = torch.cat(norms, dim=0).cpu().numpy()
    return features, norms


def infer_batch(model, use_flip_test, fusion_method, images):
    outcome_feature = model(images)
    if isinstance(outcome_feature, tuple):
        outcome_feature, outcome_norm = outcome_feature
    else:
        outcome_norm = None

    if use_flip_test:
        flipped_images = torch.flip(images, dims=[3])
        flipped_feature = model(flipped_images)
        if isinstance(flipped_feature, tuple):
            flipped_feature, flipped_norm = flipped_feature
        else:
            flipped_norm = None
        ...
        stacked_embeddings = torch.stack(
            [outcome_feature, flipped_feature], dim=0
        )
        if outcome_norm is not None:
            stacked_norms = torch.stack([outcome_norm, flipped_norm], dim=0)
        else:
            stacked_norms = None

        outcome_feature, outcome_norm = fuse_features_with_norm(
            stacked_embeddings, stacked_norms, fusion_method=fusion_method
        )

    return outcome_feature, outcome_norm
...
def identification(data_root, dataset_name, img_input_feats, save_path):
    ...
    (
        gallery_s1_templates,
        gallery_s1_subject_ids,
    ) = eval_helper_identification.read_template_subject_id_list(
        os.path.join(meta_dir, gallery_s1_record)
    )
    print(gallery_s1_templates.shape, gallery_s1_subject_ids.shape)

    (
        gallery_s2_templates,
        gallery_s2_subject_ids,
    ) = eval_helper_identification.read_template_subject_id_list(
    ...
    (
        total_templates,
        total_medias,
    ) = eval_helper_identification.read_template_media_list(
    ...
    identification_result = eval_helper_identification.evaluation(
        probe_feats, gallery_feats, mask
    )
...
def verification(data_root, dataset_name, img_input_feats, save_path):
    ...
    (
        template_norm_feats,
        unique_templates,
    ) = eval_helper_verification.image2template_feature(
    ...
    tpr_fpr_list = eval_helper_verification.get_tpr_fpr(tpr, fpr, x_labels)
    return dict(
        zip([dataset_name + "/" + str(i) for i in x_labels], tpr_fpr_list)
    )
    ...
    eval_helper_verification.write_result(
        result_files, save_path, dataset_name, label
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_root", default="/data/data/faces/IJB/insightface_helper/ijb"
    )
    ...
    print(
        "Feature Shape: ({} , {}) .".format(
            img_input_feats.shape[0], img_input_feats.shape[1]
        )
    )
