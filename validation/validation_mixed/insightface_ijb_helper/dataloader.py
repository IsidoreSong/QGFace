...
def get_IJB_info(data_root, subset_name):
    img_root = os.path.join(data_root, subset_name, "loose_crop")
    landmark_list_path = os.path.join(
        data_root,
        subset_name,
        "meta",
        f"{subset_name.lower()}_name_5pts_score.txt",
    )
    faceness_scores = []
    img_paths = []
    landmarks = []
...
def prepare_dataloader(img_list, landmarks, batch_size, num_workers=0, image_size=(112, 112), 
    ...
    image_dataset = ListDatasetWithAligner(
        img_list, landmarks, image_size=image_size, 
        image_is_saved_with_swapped_B_and_R=image_is_saved_with_swapped_B_and_R,
        path_imgidx=path_imgidx,
        path_imgrec=path_imgrec
    )
    dataloader = DataLoader(image_dataset, batch_size=batch_size, shuffle=False, drop_last=False, num_workers=num_workers, pin_memory=True)
    return dataloader
